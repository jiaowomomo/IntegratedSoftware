...
{
    public partial class XLDMatchForm : Form
    {
        HImage source_Image;
        ...
        public XLDMatchForm(XLDMatch xLDMatch)
        {
            InitializeComponent();

            hObjectViewer1.SetROICount(20);
            hObjectViewer1.SetImageHandle(ImageHandle);
            hShapeModel = xLDMatch.hShapeModel.Clone();
            hXLDCont = hShapeModel.GetShapeModelContours(1);
            numericUpDownStartAngle.Value = Convert.ToDecimal(xLDMatch.AngleStart);
            numericUpDownEndAngle.Value = Convert.ToDecimal(xLDMatch.AngleEnd);
            numericUpDownScaleMin.Value = Convert.ToDecimal(xLDMatch.ScaleMin);
            numericUpDownScaleMax.Value = Convert.ToDecimal(xLDMatch.ScaleMax);
            numericUpDownMinScore.Value = Convert.ToDecimal(xLDMatch.MinScore);
            numericUpDownMatchCount.Value = Convert.ToDecimal(xLDMatch.MatchCount);
            numericUpDownOverlap.Value = Convert.ToDecimal(xLDMatch.Overlap);
            numericUpDownPyramid.Value = Convert.ToDecimal(xLDMatch.Pyramid);
            numericUpDownGreediness.Value = Convert.ToDecimal(xLDMatch.Greediness);
            ModelImage = xLDMatch.ModelImage;
            source_Image = new HImage(ModelImage);
            hObjectViewer1.SetImage(source_Image);
            m_listROI = xLDMatch.m_listROI;
        ...
        private void ImageHandle()
        {
            ...
            if (hRegion != null)
            {
                ...
                try
                {
                    ...
                    hShapeModel = hXLDCont.CreateScaledShapeModelXld(new HTuple(Convert.ToInt32(numericUpDownPyramid.Value)), angleStart, angleExtent, new HTuple(Convert.ToDouble(numericUpDownAngleStep.Value)), Convert.ToDouble(numericUpDownScaleMin.Value), Convert.ToDouble(numericUpDownScaleMax.Value), new HTuple(Convert.ToDouble(numericUpDownScaleStep.Value)), new HTuple("auto"), "ignore_local_polarity", new HTuple(Convert.ToInt32(numericUpDownContrastMin.Value)));
                    HTuple row, column;
                    ...
                    hObjectViewer1.AddShowObject(new AutomationSystem.Halcon.ShowObject(hXLDCont.Clone()));
        ...
        private void XLDMatchForm_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < score.Length; i++)
            {
                ...
                hObjectViewer1.AddShowObject(new AutomationSystem.Halcon.ShowObject(findCont.Clone(), "green"));
                findCont.Dispose();
                textBox1.Text += "X:" + column.TupleSelect(i).ToString() + "  Y:" + row.TupleSelect(i).ToString() + "  A:" + angle.TupleSelect(i).ToString() + "  Scale:" + scale.TupleSelect(i).ToString() + "  Score:" + score.TupleSelect(i).ToString() + "\r\n";
            }
            hObjectViewer1.ResetWndCtrl(false);
        }
